...
$('#contactForm').on('submit', function(e){
	...
	function success(response) {
		swal("Thanks!", "Your message has been sent successfully, Please Check Your Email for Confirmation Of Your Order :)!", "success");
		$this.find("input, textarea").val("");
	}

    if(name.val() == "" || email.val() == "" || message.val() == "" || subject.val() == "" || email.hasClass("invalid")){
        swal("Wait !", "Please Fill in All The Feilds Correctly", "warning");
    	...
    	$.ajax({
    		type: "POST",
    		url: "process.php",
    		data: {formData: JSON.stringify({name: name.val(), email: email.val(), subject: subject.val(), message: message.val()})}
    	}).done(function(res){

    		success("Your Message Has Been Sent Successfully, Please Check Your Email For Order Confirmation, Thanks");
